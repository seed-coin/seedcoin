...
#if defined(NDEBUG)
# error "Seedcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf07a073ee3871293cca786147bfbd3b9054df6b65ce30c56cea056036e4abbae");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Seedcoin: starting difficulty is 1 / 2^12
...
{
    // Seedcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Seedcoin: 420k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 7 * 24 * 60 * 60; // Seedcoin: 7 days
static const int64 nTargetSpacing = 5 * 60; // Seedcoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Seedcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Seedcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x599eb53e814a6ce063ef1b6d80d88c61b8313c584acc850b7dae2b71c247956a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Today 15july2018, 412years since rembrandt born";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("049a72c4da6ee1d1c3cecc2f610f7507cdbb7d4f0517bb68c2b9e4de8e0dc8bf79dfa3f73d02005dab9d743faba76a94c444cda3244077dd6566466d499288b199") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1531703370;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084526073;

        if (fTestNet)
        {
            block.nTime    = 1531703353;
            block.nNonce   = 386046984;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x9eec44f502e77524c4a079023dd88780551875e0017e950a357d8a011bae5913"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc2, 0xb8, 0xdc }; //changed again randomly, was-> // Seedcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SeedcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SeedcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SeedcoinMiner(CWallet *pwallet)
{
    printf("SeedcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running SeedcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SeedcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SeedcoinMiner, pwallet));
}
